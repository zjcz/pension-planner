...
- [x] **Ops hardening**: `/health` endpoint (`HealthController` — checks SQLite connectivity via the Hikari pool, returns `{"status":"UP"}` / `503`; permitAll) + `server.shutdown: graceful`. **Rate limiting** (`RateLimitFilter`, token bucket per client IP honouring `X-Forwarded-For`, keyed per endpoint so register/login don't share a bucket; configurable via `AUTH_RATE_LIMIT_ENABLED`/`CAPACITY`/`PER_MINUTE`, defaults 20 burst / 10 per min; `429` + JSON error body). **Security headers** (`Content-Security-Policy: default-src 'self'`, `Referrer-Policy: no-referrer`, `Permissions-Policy`, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Cache-Control: no-store`). **CORS restriction** (`CORS_ALLOWED_ORIGINS` allow-list via a `CorsConfigurationSource`; empty = same-origin only, cross-origin preflights get `403`). Tests: `AuthRateLimitIntegrationTest` (per-IP limiting, per-endpoint budgets, X-Forwarded-For, non-auth paths exempt), `SecurityHeadersIntegrationTest`, `CorsConfiguredOriginIntegrationTest` + `CorsDefaultIntegrationTest` (110 tests pass). Rate limiting is disabled in tests via `src/test/resources/application.properties` (an `application.properties` file masks only its own keys and does not shadow the main `application.yml`).
...
- [x] **GraalVM native-image build** (spec §1.2): a `native` Maven profile (`native-maven-plugin` 0.10.6, `imageName pension-planner`, reachability metadata repo enabled, `fallback=false`) compiles the backend to a ~150 MB ELF binary (`mvn -Pnative -DskipTests native:compile`) that starts in ~0.4 s. Runtime hints live in `NativeRuntimeHints` (`@ImportRuntimeHints` on `PensionPlannerApplication`) for JJWT reflection (`KeysBridge`, `DefaultJwtBuilder`/`DefaultJwtParserBuilder`, claims/header builders, `Standard*Algorithms`, `JwksBridge`, …), SQLite (`org.sqlite.JDBC`, `META-INF/services/java.sql.Driver`, embedded `libsqlitejdbc.so`), and Hibernate `SQLiteDialect`. Requires the original `flyway.version=12.6.2` pin to be removed — Spring Boot's AOT `NativeImageResourceProviderCustomizer` is compiled against the managed Flyway 11.7.2 `Scanner` API, so Flyway follows Boot's managed version (SQLite dialect is built into core 11.x; the 12.x-only `flyway-database-nc-sqlite` module is not needed). Verified end-to-end: native binary runs V1–V7 migrations, register/login, pension CRUD, statement CRUD, audit history, and the audit purge-on-settings-change; all 102 JVM tests still pass. The Docker image still uses the regular JVM fat JAR.
...
- [x] **Auth UX**: registration visibility driven by `GET /api/v1/auth/config` (Login hides the Register link, Register shows a "disabled by administrator" screen when `ALLOW_REGISTRATION=false`). **Session expiry**: an axios response interceptor on 401 (excluding `/auth/login` & `/auth/register`) triggers `SessionExpiredWatcher` → clears the query cache (`markSessionExpired`) and shows a `SessionExpiredDialog` (non-dismissable) with a "Sign In" action; the misleading "Invalid username or password" is no longer shown for expired mid-session requests. **Consistent states**: shared `PageLoading` (ProgressSpinner) and `PageError` (`<Message severity="error">`) components used by the pages that do full-page loading/error (Analytics, Pension Details) plus a summary-card loading/error gate on the dashboard; all DataTables keep native `loading` + `emptyMessage` and inline error messages.
